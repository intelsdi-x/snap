...
| signed | the bool value to indicate if the plugin is signed or not |
| status | the plugin status |
| loaded_timestamp | the time plugin loaded |
...
List plugins given type, name and version
...
Unload a plugin given type, and version
...
| Parameter  | Description | 
| :--------- | :--------------- | 
| last_advertised_timestamp | the last time the metric used |
| namespace | the metric namespace |
| version | the metric version |
| policy.name | the metric policy name | 
| policy.type | the policy data type |
| policy.default | the flag to indicate if the policy is default one |
| policy.required | the bool value to indicate if the policy is mandatory |
...
| Parameter  | Description | 
| :--------- | :--------------- | 
| id | the task id defined in UUID |
| name | the task name |
| deadline | the task timeout time |
| creation_timestamp | the task creation time | 
| last_run_timestamp | the last running time of a task |
| hit_count | the number of times a task ran |
| task_state | the state of a task|
| workflow.collect.metrics | map of collecting metrics |
| workflow.collect.config | map of collecting metrics configurations |
| workflow.collect.process | array of processes used |
| workflow.collect.process.publish | array of publishers |
...
Remove an agreemen given the agreement name
