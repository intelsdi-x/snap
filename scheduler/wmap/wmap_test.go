...
func TestWorkflow(t *testing.T) {
	...
	Convey("Workflow map", t, func() {
		...
		Convey("NewWorkFlowMap()/GetRequestedMetrics()", func() {
			...
			So(wmap.CollectNode.GetRequestedMetrics(), ShouldBeEmpty)
			wmap.CollectNode.AddMetric("/foo/bar", 1)
			So(wmap.CollectNode.GetRequestedMetrics(), ShouldNotBeEmpty)
			wmap.CollectNode.GetRequestedMetrics()[0].Namespace()
			So(wmap.CollectNode.GetRequestedMetrics()[0].Namespace(), ShouldResemble, []string{"foo", "bar"})
			wmap.CollectNode.GetRequestedMetrics()[0].Version()
			So(wmap.CollectNode.GetRequestedMetrics()[0].Version(), ShouldResemble, 1)
		})
