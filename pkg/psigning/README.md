...
==============
# Security
By default, the snap Daemon has plugin signing verification enabled. To disable it or turn it to warning, the flag `--plugin-trust, -t` can be set to 0 or 2 respectively. 
...
```
snapd -k <keyringFileOrDirectory> 
snapd -t <trustLevel> -k <keyringFileOrDirectory> 
snapd -t <trustLevel> -k someDirectory/someFile.gpg
snapd -t <trustLevel> -k someDirectory/
```
Multiple keyrings (may need full path, not ~)
```
snapd -t <trustLevel> -k <keyringFile1>:<keyringFile2> 
```  
...
Loading a single plugin in snapctl
```
snapctl plugin load <pluginFile> -a <pluginFile>.asc
```
...
```    
> snapd -l 1 -k /Users/tiffany/.gnupg/pubring.gpg:/Users/tiffany/.gnupg/stuff.gpg
...
Valid signature
```
> snapd -l 1 -k /Users/tiffany/.gnupg/
...
```
```
> snapctl plugin load build/plugin/snap-collector-mock1 -a build/plugin/snap-collector-mock1.asc
Plugin loaded
Name: mock1
Version: 1
Type: collector
...
Invalid signature
```
snapctl plugin load build/plugin/snap-collector-mock2 -a build/plugin/snap-collector-mock1.asc
Error loading plugin:
...
```
> snapd -l 1 -k ~/.gnupg/pubring.gpg -t 2
...
If you only have one key, you can use the `snap.pubring` you just made as your `<keyringFile>`. For multiple you can just separate them by a colon. If you want to add future pubkeys.gpg files to an existing keyring you can do  
```
