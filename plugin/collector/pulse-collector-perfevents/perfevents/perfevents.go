...
func (p *Perfevents) CollectMetrics(mts []plugin.PluginMetricType) ([]plugin.PluginMetricType, error) {
	...
	for _, m := range mts {
		...
		events = append(events, m.Namespace()[2])
		cgroups = append(cgroups, strings.Replace(m.Namespace()[3], "_", "/", -1))
	}
...
func (p *Perfevents) GetMetricTypes() ([]plugin.PluginMetricType, error) {
	...
	cgroups, _ := list_cgroups()
	mts := []plugin.PluginMetricType{}
	mts = append(mts, set_supported_metrics("cgroup", cgroups, CGROUP_EVENTS)...)
...
func set_supported_metrics(source string, cgroups []string, events []string) []plugin.PluginMetricType {
	...
	for _, e := range events {
		for _, c := range flatten_cg_name(cgroups) {
			mts = append(mts, plugin.PluginMetricType{Namespace_: []string{"perfevents", source, e, c}})
		}
	}
	return mts
}
...
func validateNamespace(namespace []string) error {
	if len(namespace) != 4 {
		return errors.New(fmt.Sprintf("unknown metricType %s (should containt exactly 4 segments)", namespace))
	}
	if namespace[0] != "perfevents" {
		return errors.New(fmt.Sprintf("unknown metricType %s (expected first segment %s)", namespace, "perfevents"))
	}

	if namespace[1] != "cgroup" {
		return errors.New(fmt.Sprintf("unknown metricType %s (expected second segment %s)", namespace, "cgroup"))
	}
	if !namespaceContains(namespace[2], CGROUP_EVENTS) {
		return errors.New(fmt.Sprintf("unknown metricType %s (expected third segment %v)", namespace, CGROUP_EVENTS))
	}
	return nil
}
