...
// Publish sends data to a SQL server
func (s *sqlPublisher) Publish(contentType string, content []byte, config map[string]ctypes.ConfigValue) error {
	logger := log.New()
...
func (f *sqlPublisher) GetConfigPolicyNode() cpolicy.ConfigPolicyNode {
	config := cpolicy.NewPolicyNode()

	username, err := cpolicy.NewStringRule("username", true, "root")
	handleErr(err)
	username.Description = "Username to login to the SQL server"

	password, err := cpolicy.NewStringRule("password", true, "root")
	handleErr(err)
	password.Description = "Password to login to the SQL server"

	database, err := cpolicy.NewStringRule("database", true, "PULSE_TEST")
	handleErr(err)
	database.Description = "The SQL database that data will be pushed to"

	tableName, err := cpolicy.NewStringRule("table name", true, "info")
	handleErr(err)
	tableName.Description = "The SQL table within the database where information will be stored"
